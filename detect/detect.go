...
// file or directory.
func (d *Detector) DetectFiles(source, formatOpts string) ([]report.Finding, error) {
	s := semgroup.NewGroup(context.Background(), 4)
	paths := make(chan scanTarget)
	fv := NewFormatValidator(formatOpts)
	s.Go(func() error {
		defer close(paths)
	...
	for pa := range paths {
		...
		s.Go(func() error {
			...
			//
			mimeType, err := mimetype.DetectReader(bytes.NewReader(b))
			if err != nil {
				return err
			}
			//
