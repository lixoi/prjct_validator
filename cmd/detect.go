...
func init() {
	...
	detectCmd.Flags().String("format-opts", "./format-opts.json", "configuration file of format validation")
}
...
func runDetect(cmd *cobra.Command, args []string) {
	...
	formatOpts, err := cmd.Flags().GetString("format-opts")
	if err != nil {
		log.Fatal().Err(err).Msg("")
	}
	...
	if noGit {
		findings, err = detector.DetectFiles(source, formatOpts)
		if err != nil {
