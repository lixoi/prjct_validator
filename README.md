...
Исходный проект gitleaks скипает бинарные данные. Был обьеденен проект prjct_validator (ветка main) с open source проектом gitleaks. 
На текущий момент:
1. Если стоит флаг --no-git (проверка текущего дерева каталогов), то идет проверка формата файлов из списка whitelist.txt и sciplist.txt. Если есть файлы не из списка whitelist, то логируется ошибка, программа завершает работу с не нулевым кодом возврата.
2. Если не стоит флаг --no-git (проверка по дереву проекта), то идет проверка средствами git-a (по патчам): если есть бинарные данные, то логируется ошибка и программа завершает работу с не нулевым кодом возврата (whitelist не участвует). Не рекомендуется использовать. Целевое решение - контроль (в ручную) истории коммитов.

Пример запуска команды с выводом логов в консоль (для пайплайна):
>./gitleaks detect --log-level info -c ./.gitleaks.toml --source ${PASTH_TO_PROJECT}/. --no-git --verbose

Исполняемые файлы лежат в папках ./linux и ./windows

Таким образом, для работы проекта необходимы следующие конфигурационные файлы, которые (по умолчанию) должны лежать в текущей директории исполняемого файла:
- format-opts.json
- whitelist.txt
- sciplist.txt

Пример: директория test-project

При добавлении правил в .gitleaks.toml нет необходимости пересобирать проект.

Необходимо явным образом указывать путь до gitleaks.toml.

При добавлении рег. выражений необходимо правильно настраивать (на основе стат. данных) энтропию.
...
my_token_syn: GRTHGRT$^$%YGVBY^YV %^$U Y^UW%^&EW W%^HY
